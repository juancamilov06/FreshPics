...
class TrendingRemoteDataSourceTest {
    ...
    fun `given getTrendingGifs is called, when api response throws exception, then result should be successful`() = runBlockingTest {
        whenever(
    ...
    @Test
    fun `given getTrendingGifs is called, when api response is successful, then result should be successful`() = runBlockingTest {
        whenever(
            giphyAPI.getTrendingGifs(
                offset = TEST_OFFSET,
                rating = TEST_RATING,
    ...
    @Test
    fun `given getTrendingGifs is called, when api response is successful and gif not favorite in db, then result should be successful and gif favorite should be false`() = runBlockingTest {
        whenever(
            giphyAPI.getTrendingGifs(
                offset = TEST_OFFSET,
                rating = TEST_RATING,
    ...
    @Test
    fun `given getTrendingGifs is called, when api response is successful and gif is favorite in db, then result should be successful and gif favorite should be true`() = runBlockingTest {
        whenever(
            giphyAPI.getTrendingGifs(
                offset = TEST_OFFSET,
                rating = TEST_RATING,
    ...
    private fun buildDataSource() = TrendingRemoteDataSourceImpl(
        giphyAPI,
        favoritesDao
    )
